-- if isClient() then ModData.request("BetterPrisonner") end
-- ModData.getOrCreate("BetterPrisonner").PlayersStates [self.character:getUsername()] = "OnBedReversoS";
-- ModData.transmit("BetterPrisonner");
    ...
    if BPSharedStuff.tickStart % BPSharedStuff.tickControl == 0 then
        ...
        elseif BPSharedStuff.NEED_TO_CHECK == 1 then
            ...
            if  ModData.getOrCreate("BPplayerData")[player:getUsername()] == nil then
                ...
                playerdataClient.addSetPlayer(player:getDisplayName(), {
                ...
                playerdataClient.addSetPlayer(player:getDisplayName(), {
                    isSurrendering = ModData.getOrCreate("BPplayerData")[player:getDisplayName()].isRestrained, --not a erro, cause if the player are restrained they will be surrender, and if not, dont need more surrender
                    isRestrained = ModData.getOrCreate("BPplayerData")[player:getDisplayName()].isRestrained,
                });
