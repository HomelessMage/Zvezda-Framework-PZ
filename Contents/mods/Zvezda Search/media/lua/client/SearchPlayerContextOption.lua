...
function searchPlayerContextOption(player, context, worldObjects)
    ...
    -- and ModData.getOrCreate("BPplayerData")[otherPlayer:getUsername()].isSurrendering
    if otherPlayer and otherPlayer ~= playerObj and ModData.getOrCreate("BPplayerData")[otherPlayer:getUsername()].isSurrendering and not otherPlayer:isAsleep() and isClient() then
        -- local text = getText("UI_SearchStub", otherPlayer:getDisplayName());
        -- local text = getText("UI_SearchStub", otherPlayer:getDescriptor():getForename() .. " " .. otherPlayer:getDescriptor():getSurname());
