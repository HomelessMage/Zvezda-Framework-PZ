require "ISUI/UserPanel/IsSafehouseAddPlayer"
...
function ISSafehouseAddPlayerUI:populateList()
    ...
    for i=1,self.scoreboard.usernames:size() do
        ...
        local doIt = false;
        if self.changeOwnership then
			local canSee = self.player:CanSee(getPlayerFromUsername(username))
            doIt = canSee and not self.safehouse:getOwner() == username
        else
			local canSee = self.player:CanSee(getPlayerFromUsername(username))
            doIt = username ~= self.player:getUsername() and canSee and not self.safehouse:getPlayers():contains(username);
        end
        if doIt then
            local newPlayer = {};
