require "ISUI/UserPanel/IsFactionAddPlayer"
...
function ISFactionAddPlayerUI:populateList()
    ...
    for i=1,self.scoreboard.usernames:size() do
        ...
        if self.changeOwnership then
            ...
            doIt = canSee and not self.faction:isOwner(username);
        else
			local canSee = self.player:CanSee(getPlayerFromUsername(username))
            doIt = username ~= self.player:getUsername() and canSee and not self.faction:isMember(username);
        end
        if doIt then
            local newPlayer = {};
