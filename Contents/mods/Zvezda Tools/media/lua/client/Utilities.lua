...
NecropolisUtilities.adminMenu = function(player, context, worldobjects, test, items)
	...
	if getAccessLevel() ~= "" then --anything observer and up
		if sq and sq:getBuilding() then
			...
			context:addOption(getText("IGUI_GetKey"), worldobjects, DebugContextMenu.OnGetBuildingKey, player)
		end
	end
	for _,obj in ipairs(worldobjects) do --filter for what we find when we right click
		...
		if instanceof(obj, "IsoThumpable") and obj:isDoor() then
			if getAccessLevel() ~= "" then
				context:addOption(door:isLocked() and getText("IGUI_DoorUnlock") or getText("IGUI_DoorLock"), worldobjects, DebugContextMenu.OnDoorLock, door)
				context:addOption(string.format(getText("IGUI_SetDoorKeyID") .. " (%d)]", door:getKeyId()), worldobjects, DebugContextMenu.OnSetDoorKeyID, door)
			end
			if not zGood or dist > 2 then 
				return
			else
				context:addOption(getText("IGUI_Knock"), worldobjects, NecropolisUtilities.knockdoor, player, door);
			end
			return
		elseif instanceof(obj, "IsoDoor") then
			if getAccessLevel() ~= "" then
				context:addOption(obj:isLocked() and getText("IGUI_DoorUnlock") or getText("IGUI_DoorLock"), worldobjects, DebugContextMenu.OnDoorLock, door)
				context:addOption(string.format(getText("IGUI_SetDoorKeyID") .. " (%d)]", obj:getKeyId()), worldobjects, DebugContextMenu.OnSetDoorKeyID, door)
			end
			if not zGood or dist > 2 then 
				return
			else
		...
		elseif instanceof(obj, "IsoWindow") then
			if getAccessLevel() ~= "" then
				context:addOption(window:isLocked() and getText("IGUI_WindowUnlock") or getText("IGUI_WindowLock"), worldobjects, DebugContextMenu.OnWindowLock, window)
				context:addOption(window:isPermaLocked() and getText("IGUI_WindowPermUnlock") or getText("IGUI_WindowPermLock"), worldobjects, DebugContextMenu.OnWindowPermLock, window)
			end
			if not zGood or dist > 2 then 
				return
			else
...
function ISWorldMap:onRightMouseUp(x, y)
	...
	local option = context:addOption(getText("IGUI_ShowCellGrid"), self, function(self) self:setShowCellGrid(not self.showCellGrid) end)
	context:setOptionChecked(option, self.showCellGrid)

	option = context:addOption(getText("IGUI_ShowTileGrid"), self, function(self) self:setShowTileGrid(not self.showTileGrid) end)
	context:setOptionChecked(option, self.showTileGrid)

	self.hideUnvisitedAreas = self.mapAPI:getBoolean("HideUnvisited")
	option = context:addOption(getText("IGUI_HideUnvisited"), self, function(self) self:setHideUnvisitedAreas(not self.hideUnvisitedAreas) end)
	context:setOptionChecked(option, self.hideUnvisitedAreas)

	option = context:addOption(getText("IGUI_Isometric"), self, function(self) self:setIsometric(not self.isometric) end)
	context:setOptionChecked(option, self.isometric)

	-- DEV: Apply the style again after reloading ISMapDefinitions.lua
	option = context:addOption(getText("IGUI_ReapplyStyle"), self,
		function(self)
	...
	if getWorld():getMetaGrid():isValidChunk(worldX / 10, worldY / 10) then
		option = context:addOption(getText("IGUI_TeleportHere"), self, self.onTeleport, worldX, worldY)
	end
