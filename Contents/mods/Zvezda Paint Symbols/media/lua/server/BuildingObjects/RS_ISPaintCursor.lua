...
function ISPaintCursor:render(x, y, z, square)
    ...
    if self.action ~= "paintSign" or self.args.sign < 400 then
        ISPaintCursor_render(self, x, y, z, square);
    else
        if not self.floorSprite then
            self.floorSprite = IsoSprite.new()
            self.floorSprite:LoadFramesNoDirPageSimple(
                'media/ui/FloorTileCursor.png')
        end
        ...
        if self.objectIndex >= 1 and self.objectIndex <= #objects then
            ...
            if sign >= 400 and sign < 500 then
                sign = sign - 400;
                self.signSprite:LoadFramesNoDirPageSimple(
                    "treads_paintsymbols_01_" .. sign)
            elseif sign >= 500 and sign < 600 then
                sign = sign - 500;
                self.signSprite:LoadFramesNoDirPageSimple(
                    "treads_paintsymbols_02_" .. sign)
            elseif sign >= 600 and sign < 700 then
                sign = sign - 600;
                self.signSprite:LoadFramesNoDirPageSimple(
                    "treads_paintletters_01_" .. sign)
            elseif sign >= 700 and sign < 800 then
                sign = sign - 700;
                self.signSprite:LoadFramesNoDirPageSimple(
                    "treads_paintletters_02_" .. sign)
            end
            self.signSprite:RenderGhostTileColor(x, y, z, color.r, color.g,
                                                 color.b, 1.0)
